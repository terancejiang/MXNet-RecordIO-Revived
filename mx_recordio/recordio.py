...
from mx_recordio.base import _LIB
from mx_recordio.base import RecordIOHandle
from mx_recordio.base import check_call
from mx_recordio.base import c_str
...
class MXRecordIO(object):
    ...
    Examples
    ---------
    >>> record = mx.mx_recordio.MXRecordIO('tmp.rec', 'w')
    <mxnet.mx_recordio.MXRecordIO object at 0x10ef40ed0>
    >>> for i in range(5):
    ...    record.write('record_%d'%i)
    >>> record.close()
    >>> record = mx.mx_recordio.MXRecordIO('tmp.rec', 'r')
    >>> for i in range(5):
    ...    item = record.read()
    ...    print(item)
    record_0
...
class MXIndexedRecordIO(MXRecordIO):
    ...
    >>> record.close()
    >>> record = mx.mx_recordio.MXIndexedRecordIO('tmp.idx', 'tmp.rec', 'r')
    >>> record.read_idx(3)
    record_3
    ...
    def tell(self):
        ...
        Examples
        ---------
        >>> record = mx.mx_recordio.MXIndexedRecordIO('tmp.idx', 'tmp.rec', 'w')
        >>> print(record.tell())
        0
        >>> for i in range(5):
        ...     record.write_idx(i, 'record_%d'%i)
...
See mxnet.mx_recordio.pack and mxnet.mx_recordio.pack_img for example uses.
...
def pack(header, s):
    ...
    >>> id = 2574
    >>> header = mx.mx_recordio.IRHeader(0, label, id, 0)
    >>> with open(path, 'r') as file:
    ...     s = file.read()
    >>> packed_s = mx.mx_recordio.pack(header, s)
    """
...
def unpack(s):
    ...
    Examples
    --------
    >>> record = mx.mx_recordio.MXRecordIO('test.rec', 'r')
    >>> item = record.read()
    >>> header, s = mx.mx_recordio.unpack(item)
    >>> header
...
def unpack_img(s, iscolor=-1):
    ...
    >>> record = mx.mx_recordio.MXRecordIO('test.rec', 'r')
    >>> item = record.read()
    >>> header, img = mx.mx_recordio.unpack_img(item)
    >>> header
    HEADER(flag=0, label=14.0, id=20129312, id2=0)
    >>> img
    array([[[ 23,  27,  45],
...
def pack_img(header, img, quality=95, img_fmt='.jpg'):
    ...
    >>> packed_s = mx.mx_recordio.pack_img(header, img)
    """
    assert cv2 is not None
